...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get only completed or cancelled bookings (checked out guests)
    const bookings = await prisma.booking.findMany({
      where: { 
        hotelId,
        status: { in: ['completed', 'cancelled'] } // Only past bookings
      },
      include: {
        room: {
          select: {
    ...
    bookings.forEach((booking) => {
      ...
      if (!guest.lastVisit || checkOutDate > new Date(guest.lastVisit)) {
        guest.lastVisit = checkOutDate.toISOString()
      }
      
      if (!guest.firstVisit || checkInDate < new Date(guest.firstVisit)) {
        guest.firstVisit = checkInDate.toISOString()
      }
    ...
    console.error('Guest history fetch error:', error)
